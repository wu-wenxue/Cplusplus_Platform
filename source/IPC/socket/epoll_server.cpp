...
bool EpollServer::init(int port,int sock_count)
{
...
bool EpollServer::init(const char* ip,int port,int sock_count)
{
    m_max_count = sock_count;
    struct sockaddr_in server_addr;
    memset(&server_addr,0,sizeof(&server_addr));
...
int EpollServer::accept_new_client()
{
...
int EpollServer::recv_data(int sock,char* recv_buf)
{
    char buf[1024] = {0};
    int len = 0;
    int ret = 0;
...
int EpollServer::send_data(int sock,char* send_buf,int buf_len)
{
    int len = 0;
    int ret = 0;
    while(len < buf_len)
...
int EpollServer::epoll_server_wait(int time_out)
{
    int nfds = epoll_wait(m_epoll_fd,m_epoll_events,m_max_count,time_out);
}

void EpollServer::run(int time_out)
{
